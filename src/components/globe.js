const atmosphere_frag_shader = `uniform float time;
uniform float speed;
...
const atmosphere_vert_shader = `attribute float size;
...
const planet_frag_shader = `// Terrain generation parameters
uniform int type;
...
`;
const planet_vert_shader = `attribute vec3 tangent;
...
async function loadScript(txt, id, type_name) {
  const script = document.createElement("script");
  script.textContent = txt;
  script.id = id;
  script.type = type_name;
...
export default function createGlobe() {
  ...
  let ids = [
    ...
    "planet-vert-shader",
    "planet-frag-shader",
    "atmosphere-vert-shader",
    "atmosphere-frag-shader",
  ];
  loadScript(noise_functions, ids[0], shader_types[0]);
  loadScript(planet_vert_shader, ids[1], shader_types[1]);
  loadScript(planet_frag_shader, ids[2], shader_types[2]);
  loadScript(atmosphere_vert_shader, ids[3], shader_types[3]);
  loadScript(atmosphere_frag_shader, ids[4], shader_types[4]);

  const script = document.createElement("script");
  script.src = "/globe_req.js";
  document.body.appendChild(script);
}
